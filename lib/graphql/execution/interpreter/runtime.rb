    ...
    class Interpreter
      ...
      class Runtime
        ...
        def evaluate_selections(is_eager_selection, gathered_selections, selections_result, target_result, parent_object, runtime_state) # rubocop:disable Metrics/ParameterLists
          ...
          if (gathered_nodes = gathered_selections["nodes"])
            gathered_selections = gathered_selections.inject({"nodes" => gathered_nodes}) do |result, selection|
              result.store(*selection) unless selection.first == "nodes"
              result
            end
          end
